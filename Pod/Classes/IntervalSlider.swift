...
public class IntervalSlider: UIView {
  ...
  fileprivate var slider: TapSlider!
fileprivate var sources = [IntervalSliderSource]()
  fileprivate var labels = [UILabel]()
  fileprivate var marks = [UIView]()
  
  // options
  fileprivate var minimumValue: Float = 0 {
    willSet {
      if newValue < 0 || newValue > 100 { fatalError("MinimumValue must be between 0 and 100") }
    }
  }
  fileprivate var maximumValue: Float = 100 {
    willSet {
      if newValue < 0 || newValue > 100 { fatalError("MaximumValue must be between 0 and 100") }
    }
  }
  fileprivate var minimumTrackTintColor = UIColor.blue
  fileprivate var labelBottomPadding: CGFloat = 5
  fileprivate var defaultValue: Float = 0
  fileprivate var isAddMark: Bool = false
  fileprivate var thumbImage: UIImage?
  fileprivate var defaultThumbImage: UIImage {
...
//    self.slider.addTarget(self, action: Selector(("endDrag:")), for: .touchUpInside)
//    self.slider.addTarget(self, action: Selector(("endDrag:")), for: .touchUpOutside)
  ...
  private func layout(){
    self.slider.frame = CGRect(x: 0, y: self.frame.height / 2, width: self.frame.width, height: self.frame.height)
    var index: Int = 0
    for source in self.sources {
      let label = self.labels[index]
      let center = self.thumbCenter(value: source.appearanceValue)
       
      label.frame =  CGRect(x: 0, y: self.slider.frame.origin.y - (label.bounds.size.height + self.labelBottomPadding), width: label.bounds.size.width, height: label.bounds.size.height)
